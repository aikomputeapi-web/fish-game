/* Reef Fortune — original arcade fish-shooter for the browser.
...
const RTP = 0.94;                        // long-run payout ratio for catch odds
const FIRE_INTERVAL = 0.14;              // seconds between shots when holding
const BULLET_SPEED = 640;
const MAX_BOUNCES = 3;

// ---- fish species table -------------------------------------------------
// mult  : payout multiplier (win = mult * bet)
...
const SPECIES = [
  ...
  { id: 'mandarin', name: 'Neon Coral Mandarinfish', mult: 9, weight: 9, r: 30, speed: 100, kind: 'mandarin', body: '#1f6dff', belly: '#7dffd8', fin: '#ff8c1a', stripe: '#ffb63c', glow: '#4ec9ff' },
  { id: 'turtle',   name: 'Rune Turtle',   mult: 12,  weight: 7,  r: 38, speed: 55,  kind: 'turtle', body: '#3da35d', belly: '#cfe8b0', fin: '#2c7a44', stripe: '#1e5c31' },
  { id: 'serpray',  name: 'Emerald Serpent-Stingray', mult: 16, weight: 5.5, r: 48, speed: 80, kind: 'serpentray', body: '#12a352', belly: '#8affc0', fin: '#0a6e36', stripe: '#ffd23c', glow: '#4dff9a' },
  ...
  { id: 'thunder',  name: 'Thunder-Crest Hammerhead', mult: 35, weight: 3, r: 54, speed: 100, kind: 'thundershark', body: '#3d5a80', belly: '#dbe8f5', fin: '#28415f', stripe: '#ffd23c', hammer: true, glow: '#9fd8ff' },
  { id: 'shark',    name: 'Tiger Shark',   mult: 45,  weight: 2.5,r: 60, speed: 100, kind: 'shark',  body: '#6e8494', belly: '#dbe6ec', fin: '#51636f', stripe: '#3a4a54' },
  { id: 'crab',     name: 'Bomb Crab',     mult: 12,  weight: 3,  r: 34, speed: 45,  kind: 'crab',   body: '#e0483c', belly: '#ffb09e', fin: '#a82f26', stripe: '#701d17', special: 'bomb' },
...
const BOSS = { id: 'abysslord', name: 'ABYSS-LORD', mult: 250, r: 80, speed: 55, kind: 'abysslord', body: '#e8b23c', belly: '#ffe9b0', fin: '#a06a10', stripe: '#7a4a00', boss: true };

const TOTAL_WEIGHT = SPECIES.reduce((s, x) => s + x.weight, 0);
...
const state = {
  ...
  time: 0,
  spawnTimer: 0,
  bossTimer: 45,          // first boss arrives quickly so players see it
  bossActive: false,
  fish: [],
  bullets: [],
  nets: [],
...
const hud = {
  ...
  auto: el('btn-auto'), lock: el('btn-lock'), free: el('btn-free'),
...
function refreshHUD() {
  hud.balance.textContent = Math.floor(state.balance).toLocaleString();
  hud.win.textContent = Math.floor(state.totalWin).toLocaleString();
  hud.bet.textContent = bet();
  hud.free.classList.toggle('hidden', state.balance >= BETS[0]);
}
...
hud.lock.addEventListener('click', () => {
  state.lockMode = !state.lockMode;
  if (!state.lockMode) state.lockTarget = null;
  hud.lock.classList.toggle('on', state.lockMode);
  SFX.click();
});
hud.free.addEventListener('click', () => {
  state.balance += 1000;
  banner('+1000 COINS');
  SFX.bigWin();
  refreshHUD();
  if (window.grantServer) window.grantServer(1000);
});
...
canvas.addEventListener('pointerdown', e => {
  ...
  state.aim = p;
  if (state.lockMode) {
    const f = pickFishAt(p.x, p.y);
    if (f) { state.lockTarget = f; SFX.click(); return; }
  }
  state.firing = true;
});
...
function makePath(def) {
  // cubic bezier from one side of the screen to the other
  const fromLeft = Math.random() < 0.5;
  const margin = def.r + 60;
...
function spawnFish(def, opts = {}) {
  const path = opts.path || makePath(def);
  // approximate path length for constant speed
  let len = 0, prev = bezier(path, 0);
  for (let i = 1; i <= 24; i++) {
    const q = bezier(path, i / 24);
    len += Math.hypot(q.x - prev.x, q.y - prev.y);
    prev = q;
  }
  state.fish.push({
    def, path, t: 0,
    dur: len / (def.speed * rand(0.85, 1.2)),
    x: path[0].x, y: path[0].y, angle: 0,
    wag: rand(0, 6.28),
    dying: 0,
    flash: 0,
    boss: !!def.boss,
...
function cannonAngle() {
  const c = cannonPos();
  let target = state.aim;
  if (state.lockTarget && !state.lockTarget.dying) target = state.lockTarget;
  let a = Math.atan2(target.y - c.y, target.x - c.x);
  // keep the barrel pointing upward-ish
  return clamp(a, -Math.PI + 0.25, -0.25);
}

function tryFire() {
  if (state.fireCooldown > 0) return;
  if (window._authReady !== true) { SFX.denied(); state.firing = false; state.auto = false; hud.auto.classList.remove('on'); return; }
  if (state.balance < bet()) { SFX.denied(); state.firing = false; state.auto = false; hud.auto.classList.remove('on'); refreshHUD(); return; }
  state.balance -= bet();
  // server is source of truth: record the debit asynchronously;
  // on success/fail it updates state.balance to the authoritative value
  if (window.debitServer) window.debitServer(bet());
  ...
  const flashColors = ['#ffd54a', '#ff8c1a', '#ff5a1a'];
  for (let i = 0; i < 9; i++) {
...
// catch odds: expected payout per hit ≈ RTP * bet
function resolveHit(fish, bullet) {
  fish.flash = 0.12;
  SFX.hit();
  state.nets.push({ x: bullet.x, y: bullet.y, r: 10, max: 46 + bullet.bet * 0.9 + fish.def.r * 0.6, life: 0.35, maxLife: 0.35 });

  const p = RTP / fish.def.mult;
  if (Math.random() < p) killFish(fish, bullet.bet);
}

function killFish(fish, betAmt, fromBomb = false) {
  if (fish.dying) return;
  fish.dying = 0.6;
  fish.betAtKill = betAmt;
  const win = fish.def.mult * betAmt;
  state.totalWin += win;
  state.balance += win;
  // persist credit to server (async; server returns authoritative balance)
  if (window.creditServer) window.creditServer(win);

  const big = fish.def.mult >= 30;
  if (fish.boss) { banner('ABYSS-LORD DOWN! +' + win.toLocaleString()); SFX.bigWin(); SFX.boom(); state.bossActive = false; }
  else if (big) { banner('BIG WIN +' + win.toLocaleString()); SFX.bigWin(); }
  else SFX.kill();

  // coins fly to the balance counter
  const n = clamp(Math.round(3 + Math.log2(win + 1)), 4, 16);
  for (let i = 0; i < n; i++) {
    state.coins.push({
      x: fish.x + rand(-20, 20), y: fish.y + rand(-20, 20),
      vx: rand(-160, 160), vy: rand(-260, -80),
      t: 0, delay: i * 0.03, r: rand(7, 11), spin: rand(0, 6.28),
    });
    if (i < 6) SFX.coin(i);
  }
  state.texts.push({ x: fish.x, y: fish.y - fish.def.r, text: '+' + win.toLocaleString(), life: 1.2, maxLife: 1.2, big });
  ...
  for (let i = 0; i < 14; i++) {
    state.particles.push({
      ...
      color: i % 2 ? fish.def.body : '#ffffff',
    });
  }

  // bomb crab: chain-explode nearby fish
  if (fish.def.special === 'bomb' && !fromBomb) {
    SFX.boom();
    state.nets.push({ x: fish.x, y: fish.y, r: 20, max: 230, life: 0.5, maxLife: 0.5, bomb: true });
    for (const other of state.fish) {
      if (other === fish || other.dying || other.boss) continue;
      if (Math.hypot(other.x - fish.x, other.y - fish.y) < 230) killFish(other, betAmt, true);
    }
  }

  if (state.lockTarget === fish) state.lockTarget = null;
  refreshHUD();
}
...
function update(dt) {
  ...
  // spawn regular fish — keep the tank lively
  state.spawnTimer -= dt;
  const targetCount = 16;
  if (state.spawnTimer <= 0 && state.fish.filter(f => !f.dying).length < targetCount + 6) {
    spawnFish(pickSpecies());
    state.spawnTimer = state.fish.length < targetCount ? rand(0.15, 0.5) : rand(0.7, 1.6);
  }

  // boss cadence
  if (!state.bossActive) {
    state.bossTimer -= dt;
    if (state.bossTimer <= 0) { spawnBoss(); state.bossTimer = rand(75, 120); }
  }

  // firing
  state.fireCooldown -= dt;
  const wantFire = state.firing || state.auto || (state.lockTarget && !state.lockTarget.dying);
  if (wantFire) tryFire();
  ...
  for (let i = state.fish.length - 1; i >= 0; i--) {
    ...
    f.t += dt / f.dur;
    if (f.t >= 1) {
      if (f.boss) state.bossActive = false;
      if (state.lockTarget === f) state.lockTarget = null;
      state.fish.splice(i, 1);
      continue;
    }
    const pos = bezier(f.path, f.t);
    const tan = bezierTangent(f.path, f.t);
    f.x = pos.x; f.y = pos.y;
  ...
  // coins fly to the balance panel (top-left of bottom bar ≈ left-bottom)
  const sink = { x: 200, y: H - 40 };
  for (let i = state.coins.length - 1; i >= 0; i--) {
    const c = state.coins[i];
    if (c.delay > 0) { c.delay -= dt; continue; }
    c.t += dt;
    if (c.t < 0.35) {           // burst outward first
      c.x += c.vx * dt; c.y += c.vy * dt; c.vy += 500 * dt;
    } else {                    // then home to the counter
...
function drawFishAll() {
  for (const f of state.fish) {
    ...
    if (!f.dying) {
      ...
      g.textAlign = 'center';
      g.fillStyle = f.boss ? '#ffd54a' : 'rgba(255,255,255,0.75)';
      g.strokeStyle = 'rgba(0,0,0,0.6)';
      g.lineWidth = 3;
      const label = 'x' + f.def.mult;
    ...
    // lock indicator
    if (state.lockTarget === f) {
      g.save();
      g.strokeStyle = '#ff4444';
      g.lineWidth = 2.5;
      const rr = f.def.r * 1.5 + Math.sin(state.time * 8) * 4;
      g.beginPath(); g.arc(f.x, f.y, rr, 0, Math.PI * 2); g.stroke();
      for (let i = 0; i < 4; i++) {
        const a = i * Math.PI / 2 + state.time * 2;
        g.beginPath();
        g.moveTo(f.x + Math.cos(a) * rr, f.y + Math.sin(a) * rr);
        g.lineTo(f.x + Math.cos(a) * (rr + 10), f.y + Math.sin(a) * (rr + 10));
        g.stroke();
      }
      g.restore();
    }
...
function drawCannon() {
  ...
  // bet label on the dome
  g.font = 'bold 16px Arial';
  g.textAlign = 'center';
  g.textBaseline = 'middle';
  g.fillStyle = '#3a2200';
  g.fillText(String(bet()), c.x, c.y + 1);
  g.textBaseline = 'alphabetic';
}

function drawAimLine() {
  if (state.lockTarget || !state.firing) return;
  const c = cannonPos();
  const a = cannonAngle();
  g.save();
  g.globalAlpha = 0.15;
...
function render() {
  ...
  g.setTransform(viewScale, 0, 0, viewScale, viewOX, viewOY);
